            ...
            try
            {
                ...
                switch (pVal.EventType)
                {
                    ...
                    case SAPbouiCOM.BoEventTypes.et_CHOOSE_FROM_LIST:
                        ...
                        {
                            CFLInit.usercfl(oForm.UniqueID, "@FT_SHIPD", "U_loading", 0, "", "fulladd", "select code, U_Desc as fulladd from [@00018]", false);
                            BubbleEvent = false;
                        }
                        if (pVal.ItemUID == "c_discharg")
                        {
                            CFLInit.usercfl(oForm.UniqueID, "@FT_SHIPD", "U_discharg", 0, "", "fulladd", "select U_ZONE2, U_CN2, U_PORT2, U_FD2 as fulladd from [@00013]", false);
                            BubbleEvent = false;
                        }
                        if (pVal.ItemUID == "c_country")
                        {
                            CFLInit.usercfl(oForm.UniqueID, "@FT_SHIPD", "U_country", pVal.Row, "", "name", "select code, name from [@00020]", false);
                            BubbleEvent = false;
                        }
                        if (pVal.ItemUID == "c_booking")
                        {
                            ...
                            //CFLInit.usercfl(oForm.UniqueID, "@FT_SHIPD", "U_booking", pVal.Row, "", "U_bookno", "select distinct t1.U_bookno, t1.U_vessel, t2.name as port_of_loading, t3.name as port_of_discharge from [@FT_APSOC] t1 left outer join [@00018] t2 on t1.U_portload = t2.code left outer join [@00018] t3 on t1.U_dest = t3.code left outer join [@0020] t4 on t1.U_shipper = t4.code where docentry = " + docentry, false);
                            CFLInit.usercfl(oForm.UniqueID, "@FT_SHIPD", "U_booking", pVal.Row, "", "U_bookno", "select distinct t1.U_bookno from [@FT_APSOC] t1 where docentry = " + docentry, false);
                            BubbleEvent = false;
                        }
                        if (pVal.ColUID == "U_conno")
                        {
        ...
        public static void processMenuEventbefore(SAPbouiCOM.Form oForm, ref SAPbouiCOM.MenuEvent pVal, ref bool BubbleEvent)
        {
            try
            {
                ...
                if (pVal.MenuUID == "1294")
                {
                    ...
                    {
                        oMatrix = (SAPbouiCOM.Matrix)oForm.Items.Item(oForm.ActiveItem).Specific;
                        value = ((SAPbouiCOM.EditText)oMatrix.Columns.Item("U_batchno").Cells.Item(SAP.currentMatrixRow).Specific).Value.ToString();
                        if (value == "")
                            BubbleEvent = false;
                        value = ((SAPbouiCOM.EditText)oMatrix.Columns.Item("U_batchno").Cells.Item(oMatrix.RowCount).Specific).Value.ToString();
                        if (value != "")
                        {
                            BubbleEvent = false;
                        }
                ...
                else if (pVal.MenuUID == "1292")
                {
                    ...
                    {
                        oMatrix = (SAPbouiCOM.Matrix)oForm.Items.Item(oForm.ActiveItem).Specific;
                        addmatrix(oForm, oMatrix, "U_batchno", "@FT_SHIP3");
                    }
                }
