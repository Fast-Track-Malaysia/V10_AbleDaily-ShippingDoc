...
namespace FT_ADDON.AP_SO
{
    class CFLInit
    {
        ...
        {
            FT_ADDON.SAP.SBOApplication.StatusBar.SetText("Initialize CFL window...", SAPbouiCOM.BoMessageTime.bmt_Short, SAPbouiCOM.BoStatusBarMessageType.smt_Warning);
            ...
            try
            {
                ...
                {
                    sql = "select rdr1.itemcode, rdr1.dscription, rdr1.U_desc, rdr1.U_perfcl from rdr1 where " + sql;
                }
                else if (col == "U_conno" && ds == "@FT_SHIP2")
                {
                    //sql = "select t1.U_conno, t1.U_blno, t1.U_vessel, t1.U_sealno, t3.name as consize, t1.U_consize, t1.U_netw, t1.U_grossw, t1.U_measure, t1.U_batchno, t1.U_bmd, t1.U_bed, t1.U_batchrem, t1.docentry, t1.lineid from [@FT_APSOC] t1 left outer join [@0014] t3 on t1.U_consize = t3.code left outer join [@FT_SHIP2] t2 on t1.docentry = t2.U_docentry and t1.lineid = t2.U_lineid where " + sql + " and t2.docentry is null";
