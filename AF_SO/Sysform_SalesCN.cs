                    ...
                    case SAPbouiCOM.BoEventTypes.et_DOUBLE_CLICK:
                        if (pVal.ItemUID == "38")
                        {
                            if (oForm.Mode == SAPbouiCOM.BoFormMode.fm_OK_MODE)
                            {
                                ...
                                {
                                    int docentry = int.Parse(oForm.DataSources.DBDataSources.Item("RIN1").GetValue("DOCENTRY", pVal.Row - 1).ToString());
                                    int linenum = int.Parse(oForm.DataSources.DBDataSources.Item("RIN1").GetValue("LINENUM", pVal.Row - 1).ToString());

                                    InitForm.TEXT(oForm.UniqueID, docentry, linenum, pVal.Row, "RIN1", "38", oForm.DataSources.DBDataSources.Item("RIN1").GetValue("TEXT", pVal.Row - 1).ToString());
                                    BubbleEvent = false;
                                }
                            }
                        }
                        ...
                        if (pVal.ItemUID == "38")
                        {
                            if (oForm.Mode == SAPbouiCOM.BoFormMode.fm_OK_MODE)
                            {
                                if (pVal.Row >= 0 && pVal.ColUID != "256")
                                {
                                    ...
                                    //else
                                    //{
                                        InitForm.SDM(oForm.UniqueID, docentry, linenum, pVal.Row, "RIN1", "38");
                                    //}
                                }
                            }
                        }
